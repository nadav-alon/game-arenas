...
function App() {
  const [arena, setArena] = useState(new Arena<unknown, Vertex<unknown>[], [string, string][]>())
...
function ArenaForm(props: { finish: (completeArena: Arena<unknown, Vertex<unknown>[], [string, string][]>) => void }) {
  const [arena, setArena] = useState(new Arena<unknown, Vertex<unknown>[], [string, string][]>())
...
function DraggableVertex(props: {
  vertex: Vertex, verticesRefs: React.RefObject<Record<string, {
